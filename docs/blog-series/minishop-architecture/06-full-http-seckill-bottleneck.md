...
- 2026-04-27 GCP Compute Engine：`TWD 8.04`。
- 同日 Artifact Registry、Networking、VM Manager 顯示為 `TWD 0.00`。
...
## 第五組對照：Go load generator 下的 API/concurrency sweep

換成 Go HTTP load generator 後，API replica 才開始出現可觀察的 scale-out 訊號。以下測試都使用 OrbStack Docker Swarm、10k requests、12 buckets / partitions、`maxProbe=1`、single Redpanda broker。

先用「API replica 與 concurrency 成比例上升」測：

| API replicas | concurrency | run id | HTTP accepted RPS | Kafka durable accepted | p95 latency |
|---:|---:|---|---:|---:|---:|
| 1 | 200 | `orb_gohttp_api1_c200_10k_20260428T111844Z` | `9,430/s` | 10000 | `61.89ms` |
| 2 | 400 | `orb_gohttp_api2_c400_10k_20260428T111941Z` | `15,793/s` | 10000 | `61.26ms` |
| 4 | 800 | `orb_gohttp_api4_c800_10k_20260428T112039Z` | `11,026/s` | 10000 | `188.82ms` |

這組結果表示 API=1 到 API=2 有明顯提升，但 API=4 / concurrency=800 已經開始排隊，tail latency 惡化，throughput 反而下降。

接著固定 API=2，掃 concurrency：

| API replicas | concurrency | run id | HTTP accepted RPS | p95 latency | 備註 |
|---:|---:|---|---:|---:|---|
| 2 | 200 | `orb_gohttp_api2_c200_10k_20260428T115833Z` | `14,472/s` | `28.99ms` | latency 最低 |
| 2 | 300 | `orb_gohttp_api2_c300_10k_20260428T115921Z` | `19,770/s` | `36.24ms` | 目前最佳 throughput / latency trade-off |
| 2 | 400 | `orb_gohttp_api2_c400_10k_20260428T120009Z` | `15,752/s` | `62.18ms` | Prometheus durable delta 取樣異常，但 topic offsets 正常 |
| 2 | 500 | `orb_gohttp_api2_c500_10k_20260428T120057Z` | `8,045/s` | `200.24ms` | 已過載 |
| 2 | 600 | `orb_gohttp_api2_c600_10k_20260428T120149Z` | `13,131/s` | `146.89ms` | tail latency 明顯惡化 |

目前 OrbStack 的 sweet spot 是 API=2 / concurrency=300。這不是 production 絕對值，而是下一輪 GCP 實驗的起點。GCP 應先固定 producer 參數，再做 API/concurrency 矩陣，避免把 linger/batch tuning 與 replica scaling 混在一起。

### 3 Redpanda + 24 partitions 對照

後續也在 OrbStack Swarm 用臨時 override 跑了 3 個 Redpanda broker，並把 seckill request/result/DLQ topic 提到 24 partitions。這組測試的 topic replication factor 仍是 `1`，所以它測的是「3 broker cluster + 24 partitions」對 full HTTP path 的影響，不是 RF=3 複寫成本。

| Redpanda brokers | partitions | API replicas | concurrency | run id | HTTP accepted RPS | Kafka durable accepted | p95 latency |
|---:|---:|---:|---:|---|---:|---:|---:|
| 3 | 24 | 1 | 200 | `orb_rp3_p24_api1_c200_maxprobe1_20260428T133017Z` | `7,970/s` | 10000 | `69.11ms` |
| 3 | 24 | 2 | 300 | `orb_rp3_p24_api2_c300_maxprobe1_20260428T133253Z` | `9,884/s` | 10000 | `82.07ms` |

對照前面的單 broker / 12 partitions：

- API=1/c200：`9,430/s` -> `7,970/s`
- API=2/c300：`19,770/s` -> `9,884/s`

這表示在單台 OrbStack VM 上，把 Redpanda broker 數與 partitions 數一起提高，並沒有改善 full HTTP throughput，反而顯著降低。原因不是 durable publish 失敗，兩組 Kafka durable accepted 都是 `10000/10000`；更可能是本機單 VM 上多 broker + 更多 partitions 帶來額外 broker/client metadata、connection、scheduler、partition bookkeeping 成本。這個結果也提醒：multi-broker 必須放到真正多 VM / 多磁碟 / 多 CPU 的環境才有意義；在單機 OrbStack 上測 3 broker 不能代表 production multi-broker scale-out。
...
若 API=4 在 `400` 以上仍 tail latency 惡化，下一步應優先檢查 Swarm VIP / routing mesh / broker ack queue，而不是繼續加 API replicas。

## 第六組對照：GCP Swarm API/concurrency sweep

GCP 實驗使用 commit / image tag `d088abe63d13`，cluster 為 1 台 `e2-small` manager + 3 台 `e2-custom-2-8192` worker。Manager 維持 drain，benchmark runner 跑在 worker 上，避免小 manager 變成 load generator 瓶頸。
...
GCP 的結果與 OrbStack 不同：API=1/c200 反而是這三點裡 throughput 最高、tail latency 最低的點。增加 API replicas 並沒有帶來 scale-out；Kafka durable accepted 仍維持 `10000/10000`，表示 accepted work 最終都有完成 Kafka broker ack，但入口 HTTP path 的 tail latency 隨著 replicas/concurrency 上升快速惡化。

這代表目前 GCP Swarm topology 下，下一個最有價值的隔離實驗不是繼續加 API replicas，而是拆開：

- Swarm service VIP / routing mesh fanout。
- API task placement 與 runner placement。
- single Redpanda broker ack queue。
- published port vs single task IP vs service VIP。

Artifact 已拉回本機：

```text
benchmark-results/gcp_full_http_api_scale_20260428
```

### partitions=24 試跑結果

接著固定 `API=1`、`concurrency=200`、`maxProbe=1`，只把 buckets / topic partitions 從 `12` 提到 `24`。這組沒有跑到 HTTP throughput，因為 seckill worker 啟動時失敗：

```text
org.apache.kafka.common.errors.InvalidPartitionsException:
Can not increase partition count due to FD limit
```

細節：

- `inventory.seckill.requested` / `inventory.seckill.result` 可以手動建成 24 partitions。
- 失敗點在 worker 啟動時的 `ensureTopics()`，它會確保 Kafka Streams 內部 changelog/state topics。
- Redpanda container 內 `ulimit -n` 是 `1024`，`topic_partitions_per_shard` 是 `1000`。
- 單 broker / 單 VM 在 24 partitions + Kafka Streams internal topics 下先撞到 file descriptor / partition resource limit。

所以目前不能把「增加 partitions」直接視為 free throughput tuning。下一步如果要測 partitions > 12，必須先處理 Redpanda broker 資源限制，例如提高 container/VM FD limit、降低內部 topic 數量、或改成 multi-broker Redpanda。否則 partitions 增加會先讓 worker 無法穩定啟動。
...
| full HTTP, Go HTTP loadgen, API=2/c300 | 10k external HTTP requests | `19.8k/s` HTTP accepted | OrbStack 目前最佳點 |
| GCP full HTTP, Go HTTP loadgen, API=1/c200 | 10k external HTTP requests | `6.7k/s` HTTP accepted | GCP Swarm 目前最佳點 |
| GCP partitions=24 | worker startup | failed before benchmark | 單 broker FD / partition resource limit 先卡住 |
